  ...
  <s-card :title="$t('send.form')">
    ...
    >
      ...
      <el-form-item prop="pass"> 
        ...
        v-model="form.pass"
        :placeholder="$t('create.pass')"></el-input>
      </el-form-item>
      <el-form-item>
        <el-input
          v-model="form.memo"
...
export default {
  ...
  data() {
    ...
    const validateAddr = (rule, value, callback) => {
      ...
      addressArr.forEach(i=> {
        if (!webc.account.isValidAddress(i)) {
  ...
  methods: {
    ...
    async sends() {
      ...
      const addressArr = this.form.address.split(",")
      const sendAmount = BigNumber(this.form.amount)
            .times(BigNumber(10).pow(6))
            .toFixed();
      try {
        await addressArr.reduce(async (backFc,i)=> {
          await backFc;
          const params = {
            memo: this.form.memo,
            address: i,
          ...
          const currentTxStatus = await this.singleSendResult(params)
          if (!currentTxStatus) {
            throw Error() 
          }
        },undefined)
    ...
    async singleSendResult(params) {
      let res = "";
      try {
        res = await this.$store.dispatch("transactions/sendMore", params);
      } catch (e) {
        this.$message({
          type: "error",

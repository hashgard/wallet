    ...
    <div class="table-header && hover">
      ...
      <div class="header-rewards">
        <p v-if="get(delegation, 'shares')">
          <span v-if="!isEmpty(reward) || !isEmpty(rewardGGT)">
            <span>{{numeral(reward.amount).format('0,0.[0000]')}}{{reward.denom}}</span>
            <span>/{{numeral(rewardGGT.amount).format('0,0.[0000]')}}{{rewardGGT.denom}}</span>
          </span>
...
export default {
  ...
  computed: {
    ...
    viewToken() {
      const token = { denom: "ugard", amount: this.v.tokens };
    ...
    rewardGGT() {
      ...
      if (!isEmpty(result)) {
        return getViewToken(get(result, "result.1"));
      }
    },
    myToken() {
      // myShares
      ...
      const myToken = BigNumber(this.v.tokens)
        ...
        .toString();
      return getViewToken({ denom: "ugard", amount: myToken });
    }
  },
  methods: {
    get,

...
import ggtLogo from '@/assets/ggt.svg';
...
export const sendTx = async function (context, pass, type, msg, msgs) {
  ...
  let from;
  // if action is draw
  if (!isEmpty(sessionStorage.getItem('draw'))) {
    // fauceat address
    from = "gard1s6auwlcevspesynw44vx23e3zhuz7as9ulz56l"
  } else {
    from = context.rootGetters['account/currentAddress'];
  }
  ...
  if (!isEmpty(mathAccount)) {
    if (!isEmpty(sessionStorage.getItem('draw'))) {
      ...
      try {
        const privateKey = "b97e5810ccf81a20274798c6886c980de48b3bf25d8427375462ff0f6b9d562f"
        const signature = webc.tx.sign(stdTx.GetSignBytes(), privateKey);
...
const getTxPara = (from, type, accState, nodeInfo, msg, msgs) => {
  ...
  const feesAmount = {
    ...
    "MsgDappCreateGrid": "1000000",
    "MsgDappDeposit": "1000000",
    "MsgDappWithdraw": "1000000"
  }
  return {

...
export default {
  ...
  getters: {
    ...
    gardBalance: function (state) {
      const gard = state.balance.find(i => i.denom === 'ugard') || {
        amount: '0',
        denom: 'ugard'
      };
      const res = {
  ...
  actions: {
    ...
    fetchBalance: async function (context) {
      ...
      if (!isEmpty(data)) {
        ...
        data.result.sort(i => (i.denom === 'ugard' ? -1 : 1));
        context.commit('setBalance', data.result);
    ...
    fetchRewardsPool: async function (context) {
      const address = 'gard1dkrahrz83mz73dsm93peqz3yjjqeagcye357pj';
      const {
        data
      } = await ajax.get(`bank/balances/${address}`);
      if (isEmpty(data)) {

...
export default {
  ...
  actions: {
    ...
    drawGard: async function (context) {
      const address = context.rootGetters['account/currentAddress'];
      const msg = {
        type: "cosmos-sdk/MsgCallContract",

        sender: "gard1s6auwlcevspesynw44vx23e3zhuz7as9ulz56l",
        contract_address: "contract848454cc8870fee4e80ca870b2b9eca5c1e427ed",
        contract_param: `string:send,[string:${address}]`,
        contract_version: ""
      };
      const {
        data
      } = await sendTx(context, "", 'hg-custom', msg);
      return Promise.resolve(data);
    },

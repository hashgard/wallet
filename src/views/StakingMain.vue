<template>
  <div class="staking-container">
    <div class="staking-top">
      <div>
        <h3 v-if="!isEmpty(delegations) && !isEmpty(validatorMap)">{{ numeral(totalDelegation.amount).format('0,0.[0000]') }} GARD</h3>
        <p>{{ $t('staking.delegationTotal') }}</p>
      </div>
      <div>
        <h3 v-if="!isEmpty(rewards)">
          {{ numeral(reward.amount).format('0,0.[0000]') }} GARD
        </h3>
        <h3 v-if="!isEmpty(rewards)">
          {{ numeral(rewardGGT.amount).format('0,0.[0000]') }} GGT
        </h3>
...
export default {
  ...
  computed: {
    ...
    totalDelegation() {
      ...
      return getViewToken({ denom: "ugard", amount: res.toFixed() });
    },
    reward() {
      return getViewToken(this.rewards[0]);
    },
    rewardGGT() {
      return getViewToken(this.rewards[1]);
    },
    viewDelegations() {
